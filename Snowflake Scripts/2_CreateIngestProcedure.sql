CREATE OR REPLACE PROCEDURE APP_DB.APP_SCHEMA.IMPORT_PRICES("FILE_PATH" VARCHAR(16777216))
RETURNS VARIANT
LANGUAGE PYTHON
...
def main(session, file_path):
 with SnowflakeFile.open(file_path, ''rb'') as f:
    ...
    current_dates = session.sql(''SELECT DISTINCT "LIST_DATE" FROM PRICE_LISTS'').to_pandas()
    ...
    df2.write.mode("append").save_as_table("PRICE_LISTS")
